...
internal class JSONObjectBuilder : IJSONBuilder<JToken>
{
    ...
    private JToken PickUpCurrentToken(string path, JToken? prev, Key currentKey)
    {
        ...
        {
            var jt = prev == null ? (root ??= new JArray()) : (prev[currentKey.Name] ??= new JArray());
            if (jt is JArray ja)
            {
        ...
        {
            var jt = prev == null ? (root ??= new JObject()) : (prev[currentKey.Name] ??= new JObject());
            if (jt is JObject jo)
            {
                return jo;
            }
    ...
    {
        if ((root != null && !rootKey.IsArrayItem) || (root != null && rootKey.IsArrayItem && root is not JArray))
        {
