...
internal class ConcurrentObjectRegistry : IObjectRegistry
{
    ...
    public bool TryGetObjectHandle(string key, [MaybeNullWhen(false)] out IObjectHandle handle)
    {
    ...
    public bool TryGetObject(string key, [MaybeNullWhen(false)] out object value)
    {
    ...
    public bool TryGetObject<T>(string key, [MaybeNullWhen(false)] out T value)
    {
        if (TryGetObjectHandle(key, out var handle))
        {
    ...
    private class ObjectHandle : AbstractDisposable, IObjectHandle
    {
        ...
        {
            this.cache.Remove(Key, out var _);
